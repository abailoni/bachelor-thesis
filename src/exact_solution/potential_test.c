...
int check_split_op(check_struct_split_op *ck_val)
{
	...
	if (  ck_val->t>=count*dt_anim)
	{
		...
		//Save animation:
		fprintf(F_ANIM,"%g;%g", norma, ck_val->t);
		for(int n=0;n<N;n++)
...
int main(int argc, char const *argv[])
{
	...
	for (int i = 0; i < N_width; ++i)
	{
		...
		sprintf(name_file_prob,"matplot/dati/prob_pot/prob_%4.4g_%d.csv",random_num,i); 
		sprintf(name_file_anim,"matplot/dati/anim_pot/anim_%4.4g_%d.csv",random_num,i); 
