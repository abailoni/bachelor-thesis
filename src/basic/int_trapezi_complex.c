...
/////////////////////////////////////////
// L'integrale è fatto comunque su una variabile reale ma l'argomento può essere complesso (funzione che va da R in C)
...
complex double int_trapezi_complex(double *estremi, double precisione, void *params, complex double (*fun) (double,void*)) 
{ 
	...
	// 	complex double sum=(x_max-x_min)*(fun(x_max,params)+fun(x_min,params))/2;

	// 	//Continuo finché non raggiungo la precisione:
	// 	complex double new_sum, total_sum, old_sum=sum;
	// 	double h=(x_max-x_min)/2;
	...
	// } else { 
		complex double sum=(fun(x_max,params)+fun(x_min,params))/2;
		double h=precisione;
