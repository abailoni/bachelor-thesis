...
double phase_shift(double E, int l)
{
	...
	{
		// Trovo J:
		double estremi[2], precisione_int=1e-2;
...
{
	// Phase-shift:
	double precisione_sez=5e-6;
	for(double E=0.05; E<1; E+=0.001)
	{
		double sigma_tot=0;
		double k_rid=sqrt(E/alpha);
		int l=0;
		for (l=0;1;l++){
			...
			// if ( (fabs((sigma_tot-sigma_tot_old)/sigma_tot_old)<precisione_sez && l>3) || l>100)
			if (l>80)
				break;
		}
		printf("%g\t%g\t%d\n",E,sigma_tot*sigma*sigma,l);
		fprintf(stderr, ".",l);
	}
	fprintf(stderr, "\n");
	return 0;
