...
void split_operator_complex_potential(int N, double dt, double L, FFT_vect *funz_onda, void *output, void *pot_params, double (*potenziale)(double,void*), void *imag_pot_params, double (*imag_potenziale)(double,void*), double *check_params, int (*check_fun)(check_struct_split_op*) )
{
	double dx=L/N;
	...
	while (check_fun(&ck_val)==0) {
		...
		for (int i = 0; i < N; ++i)
		{
			complex double part1 = cexp(-dt*(I*potenziale(funz_onda->x[i],pot_params) +imag_potenziale(funz_onda->x[i],imag_pot_params)));
			REAL(ro,i)= creal(part1);
			IMAG(ro,i)= cimag(part1);
		}
		...
		for (int i = 0; i < N; ++i)
		{
			...
			REAL(xi,i) =   cos(dt * zeta * delta_K*delta_K);
			IMAG(xi,i) = - sin(dt * zeta * delta_K*delta_K);
		}
		product_array(&trasformata,xi);

		// Antitrasformo:
		inverse_FFT_2(trasformata,funz_onda);
		free_FFT(&trasformata);
