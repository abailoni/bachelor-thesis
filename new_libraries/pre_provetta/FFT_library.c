...
//------------------------------------------------
void free_FFT(FFT_vect *vettore)
{
	free(vettore->main),free(vettore->x),free(vettore->x_sym);
}
...
void inverse_FFT(FFT_vect vector_k, FFT_vect *vector_x)
{
	...
	double dK=2*M_PI/(N*vector_k.step);
	...
	// Calcolo le posizioni k:
	for (int i = 0; i < N; ++i)
		vector_x->x[i]=dK*i, vector_x->x_sym[i]=dK * (i <= N/2 ? i : -(N-i));
}
